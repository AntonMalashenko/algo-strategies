"""S012 Gate 1 — anchored walk-forward + robustness. Never touches the reserved OOS tail.

Mirrors backtest/wf_funding_carry.py (S009 E4): the config for each test year is
chosen ONLY on the years before it (anchored train window), so the stitched
test-year series is an honest out-of-sample-choice track record. The reserved
OOS tail (>= XSectMomentumConfig.reserved_oos_start) is excluded throughout.

Run from the algo repo root:

    python backtest/wf_xsect_momentum.py
"""
import sys
...
from strategies.xsect_momentum import (  # noqa: E402
    MS_PER_DAY, XSectMomentumConfig, load_panels, run_backtest,
)
...
def main():
    base = XSectMomentumConfig(taker_fee_per_side=TAKER)
    ...
    for lb in [7, 14, 30, 60, 90]:
        for n in [2, 3, 5]:
            grid.append(dict(lookback_days=lb, top_n=n, bottom_n=n, min_universe=2 * n))
    ...
    for Y in test_years:
        ...
        stitched.append(tr)
        cfgtxt = f"lb{bg['lookback_days']} n{bg['top_n']}"
    ...
    print("\nSTITCHED walk-forward (2022..pre-OOS, out-of-sample choice):")
    ...
    for lb in [7, 14, 30, 60, 90]:
        row = f"{lb:>5}"
        for n in [2, 3, 5]:
            out, _ = run_backtest(close, funding, base.with_(lookback_days=lb, top_n=n, bottom_n=n, min_universe=2*n))
